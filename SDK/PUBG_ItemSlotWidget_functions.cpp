// PUBG FULL SDK - Generated By Respecter (5.3.4.10 [04/03/2019]) SDK

#ifdef _MSC_VER
	#pragma pack(push, 0x8)
#endif

#include "PUBG_ItemSlotWidget_parameters.hpp"

namespace SDK
{
//---------------------------------------------------------------------------
//Functions
//---------------------------------------------------------------------------

// Function ItemSlotWidget.ItemSlotWidget_C.IsFocusable
// (Event)
// Parameters:
// bool                           ReturnValue                    (Parm, OutParm, ZeroConstructor, ReturnParm, IsPlainOldData)

bool UItemSlotWidget_C::IsFocusable()
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.IsFocusable");

	UItemSlotWidget_C_IsFocusable_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;

	return params.ReturnValue;
}


// Function ItemSlotWidget.ItemSlotWidget_C.InputLB
// (Event)
// Parameters:
// bool                           ReturnValue                    (Parm, OutParm, ZeroConstructor, ReturnParm, IsPlainOldData)

bool UItemSlotWidget_C::InputLB()
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.InputLB");

	UItemSlotWidget_C_InputLB_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;

	return params.ReturnValue;
}


// Function ItemSlotWidget.ItemSlotWidget_C.InputLT
// (Event)
// Parameters:
// bool                           ReturnValue                    (Parm, OutParm, ZeroConstructor, ReturnParm, IsPlainOldData)

bool UItemSlotWidget_C::InputLT()
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.InputLT");

	UItemSlotWidget_C_InputLT_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;

	return params.ReturnValue;
}


// Function ItemSlotWidget.ItemSlotWidget_C.InputRB
// (Event)
// Parameters:
// bool                           ReturnValue                    (Parm, OutParm, ZeroConstructor, ReturnParm, IsPlainOldData)

bool UItemSlotWidget_C::InputRB()
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.InputRB");

	UItemSlotWidget_C_InputRB_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;

	return params.ReturnValue;
}


// Function ItemSlotWidget.ItemSlotWidget_C.InputRT
// (Event)
// Parameters:
// bool                           ReturnValue                    (Parm, OutParm, ZeroConstructor, ReturnParm, IsPlainOldData)

bool UItemSlotWidget_C::InputRT()
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.InputRT");

	UItemSlotWidget_C_InputRT_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;

	return params.ReturnValue;
}


// Function ItemSlotWidget.ItemSlotWidget_C.GetFocusingChildWidget
// (Event)
// Parameters:
// class UUserWidget*             ReturnValue                    (Parm, OutParm, ZeroConstructor, ReturnParm, IsPlainOldData)

class UUserWidget* UItemSlotWidget_C::GetFocusingChildWidget()
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.GetFocusingChildWidget");

	UItemSlotWidget_C_GetFocusingChildWidget_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;

	return params.ReturnValue;
}


// Function ItemSlotWidget.ItemSlotWidget_C.InputA
// (Event)
// Parameters:
// bool                           ReturnValue                    (Parm, OutParm, ZeroConstructor, ReturnParm, IsPlainOldData)

bool UItemSlotWidget_C::InputA()
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.InputA");

	UItemSlotWidget_C_InputA_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;

	return params.ReturnValue;
}


// Function ItemSlotWidget.ItemSlotWidget_C.InputB
// (Event)
// Parameters:
// bool                           ReturnValue                    (Parm, OutParm, ZeroConstructor, ReturnParm, IsPlainOldData)

bool UItemSlotWidget_C::InputB()
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.InputB");

	UItemSlotWidget_C_InputB_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;

	return params.ReturnValue;
}


// Function ItemSlotWidget.ItemSlotWidget_C.InputX
// (Event)
// Parameters:
// bool                           ReturnValue                    (Parm, OutParm, ZeroConstructor, ReturnParm, IsPlainOldData)

bool UItemSlotWidget_C::InputX()
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.InputX");

	UItemSlotWidget_C_InputX_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;

	return params.ReturnValue;
}


// Function ItemSlotWidget.ItemSlotWidget_C.InputY
// (Event)
// Parameters:
// bool                           ReturnValue                    (Parm, OutParm, ZeroConstructor, ReturnParm, IsPlainOldData)

bool UItemSlotWidget_C::InputY()
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.InputY");

	UItemSlotWidget_C_InputY_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;

	return params.ReturnValue;
}


// Function ItemSlotWidget.ItemSlotWidget_C.GetDownWidget
// (Event)
// Parameters:
// class UUserWidget*             ReturnValue                    (Parm, OutParm, ZeroConstructor, ReturnParm, IsPlainOldData)

class UUserWidget* UItemSlotWidget_C::GetDownWidget()
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.GetDownWidget");

	UItemSlotWidget_C_GetDownWidget_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;

	return params.ReturnValue;
}


// Function ItemSlotWidget.ItemSlotWidget_C.GetLeftWidget
// (Event)
// Parameters:
// class UUserWidget*             ReturnValue                    (Parm, OutParm, ZeroConstructor, ReturnParm, IsPlainOldData)

class UUserWidget* UItemSlotWidget_C::GetLeftWidget()
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.GetLeftWidget");

	UItemSlotWidget_C_GetLeftWidget_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;

	return params.ReturnValue;
}


// Function ItemSlotWidget.ItemSlotWidget_C.GetRightWidget
// (Event)
// Parameters:
// class UUserWidget*             ReturnValue                    (Parm, OutParm, ZeroConstructor, ReturnParm, IsPlainOldData)

class UUserWidget* UItemSlotWidget_C::GetRightWidget()
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.GetRightWidget");

	UItemSlotWidget_C_GetRightWidget_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;

	return params.ReturnValue;
}


// Function ItemSlotWidget.ItemSlotWidget_C.GetUpWidget
// (Event)
// Parameters:
// class UUserWidget*             ReturnValue                    (Parm, OutParm, ZeroConstructor, ReturnParm, IsPlainOldData)

class UUserWidget* UItemSlotWidget_C::GetUpWidget()
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.GetUpWidget");

	UItemSlotWidget_C_GetUpWidget_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;

	return params.ReturnValue;
}


// Function ItemSlotWidget.ItemSlotWidget_C.Down
// (Event)
// Parameters:
// bool                           ReturnValue                    (Parm, OutParm, ZeroConstructor, ReturnParm, IsPlainOldData)

bool UItemSlotWidget_C::Down()
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.Down");

	UItemSlotWidget_C_Down_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;

	return params.ReturnValue;
}


// Function ItemSlotWidget.ItemSlotWidget_C.IsFocus
// (Event)
// Parameters:
// bool                           ReturnValue                    (Parm, OutParm, ZeroConstructor, ReturnParm, IsPlainOldData)

bool UItemSlotWidget_C::IsFocus()
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.IsFocus");

	UItemSlotWidget_C_IsFocus_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;

	return params.ReturnValue;
}


// Function ItemSlotWidget.ItemSlotWidget_C.Left
// (Event)
// Parameters:
// bool                           ReturnValue                    (Parm, OutParm, ZeroConstructor, ReturnParm, IsPlainOldData)

bool UItemSlotWidget_C::Left()
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.Left");

	UItemSlotWidget_C_Left_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;

	return params.ReturnValue;
}


// Function ItemSlotWidget.ItemSlotWidget_C.Right
// (Event)
// Parameters:
// bool                           ReturnValue                    (Parm, OutParm, ZeroConstructor, ReturnParm, IsPlainOldData)

bool UItemSlotWidget_C::Right()
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.Right");

	UItemSlotWidget_C_Right_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;

	return params.ReturnValue;
}


// Function ItemSlotWidget.ItemSlotWidget_C.SetFocus
// (Event)
// Parameters:
// bool*                          NewFocus                       (Parm, ZeroConstructor, IsPlainOldData)
// bool                           ReturnValue                    (Parm, OutParm, ZeroConstructor, ReturnParm, IsPlainOldData)

bool UItemSlotWidget_C::SetFocus(bool* NewFocus)
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.SetFocus");

	UItemSlotWidget_C_SetFocus_Params params;
	params.NewFocus = NewFocus;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;

	return params.ReturnValue;
}


// Function ItemSlotWidget.ItemSlotWidget_C.Up
// (Event)
// Parameters:
// bool                           ReturnValue                    (Parm, OutParm, ZeroConstructor, ReturnParm, IsPlainOldData)

bool UItemSlotWidget_C::Up()
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.Up");

	UItemSlotWidget_C_Up_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;

	return params.ReturnValue;
}


// Function ItemSlotWidget.ItemSlotWidget_C.GetSlotItem
// ()
// Parameters:
// TScriptInterface<class USlotInterface> SlotItem                       (Parm, OutParm, ZeroConstructor, IsPlainOldData)

void UItemSlotWidget_C::GetSlotItem(TScriptInterface<class USlotInterface>* SlotItem)
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.GetSlotItem");

	UItemSlotWidget_C_GetSlotItem_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;

	if (SlotItem != nullptr)
		*SlotItem = params.SlotItem;
}


// Function ItemSlotWidget.ItemSlotWidget_C.GetSlotContainer
// ()
// Parameters:
// TScriptInterface<class USlotContainerInterface> SlotContainer                  (Parm, OutParm, ZeroConstructor, IsPlainOldData)

void UItemSlotWidget_C::GetSlotContainer(TScriptInterface<class USlotContainerInterface>* SlotContainer)
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.GetSlotContainer");

	UItemSlotWidget_C_GetSlotContainer_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;

	if (SlotContainer != nullptr)
		*SlotContainer = params.SlotContainer;
}


// Function ItemSlotWidget.ItemSlotWidget_C.RaiseLeaveEvent
// ()

void UItemSlotWidget_C::RaiseLeaveEvent()
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.RaiseLeaveEvent");

	UItemSlotWidget_C_RaiseLeaveEvent_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function ItemSlotWidget.ItemSlotWidget_C.RaiseEnterEvent
// ()

void UItemSlotWidget_C::RaiseEnterEvent()
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.RaiseEnterEvent");

	UItemSlotWidget_C_RaiseEnterEvent_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function ItemSlotWidget.ItemSlotWidget_C.PutSeltItem
// ()

void UItemSlotWidget_C::PutSeltItem()
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.PutSeltItem");

	UItemSlotWidget_C_PutSeltItem_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function ItemSlotWidget.ItemSlotWidget_C.StopSelfPutMode
// ()

void UItemSlotWidget_C::StopSelfPutMode()
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.StopSelfPutMode");

	UItemSlotWidget_C_StopSelfPutMode_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function ItemSlotWidget.ItemSlotWidget_C.StartSelfPutMode
// ()
// Parameters:
// bool                           bResult                        (Parm, OutParm, ZeroConstructor, IsPlainOldData)
// int                            StartWeaponIndex               (Parm, OutParm, ZeroConstructor, IsPlainOldData)

void UItemSlotWidget_C::StartSelfPutMode(bool* bResult, int* StartWeaponIndex)
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.StartSelfPutMode");

	UItemSlotWidget_C_StartSelfPutMode_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;

	if (bResult != nullptr)
		*bResult = params.bResult;
	if (StartWeaponIndex != nullptr)
		*StartWeaponIndex = params.StartWeaponIndex;
}


// Function ItemSlotWidget.ItemSlotWidget_C.IsSelfPutMode
// ()
// Parameters:
// bool                           bIsSelfPutMode                 (Parm, OutParm, ZeroConstructor, IsPlainOldData)

void UItemSlotWidget_C::IsSelfPutMode(bool* bIsSelfPutMode)
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.IsSelfPutMode");

	UItemSlotWidget_C_IsSelfPutMode_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;

	if (bIsSelfPutMode != nullptr)
		*bIsSelfPutMode = params.bIsSelfPutMode;
}


// Function ItemSlotWidget.ItemSlotWidget_C.ProcessPressedY
// ()

void UItemSlotWidget_C::ProcessPressedY()
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.ProcessPressedY");

	UItemSlotWidget_C_ProcessPressedY_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function ItemSlotWidget.ItemSlotWidget_C.ProcessPressedA
// ()

void UItemSlotWidget_C::ProcessPressedA()
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.ProcessPressedA");

	UItemSlotWidget_C_ProcessPressedA_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function ItemSlotWidget.ItemSlotWidget_C.InitItemSlot
// ()

void UItemSlotWidget_C::InitItemSlot()
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.InitItemSlot");

	UItemSlotWidget_C_InitItemSlot_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function ItemSlotWidget.ItemSlotWidget_C.OnInputBPressed
// ()

void UItemSlotWidget_C::OnInputBPressed()
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.OnInputBPressed");

	UItemSlotWidget_C_OnInputBPressed_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function ItemSlotWidget.ItemSlotWidget_C.OnInputYReleased
// ()

void UItemSlotWidget_C::OnInputYReleased()
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.OnInputYReleased");

	UItemSlotWidget_C_OnInputYReleased_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function ItemSlotWidget.ItemSlotWidget_C.OnInputYPressed
// ()

void UItemSlotWidget_C::OnInputYPressed()
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.OnInputYPressed");

	UItemSlotWidget_C_OnInputYPressed_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function ItemSlotWidget.ItemSlotWidget_C.OnInputAReleased
// ()

void UItemSlotWidget_C::OnInputAReleased()
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.OnInputAReleased");

	UItemSlotWidget_C_OnInputAReleased_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function ItemSlotWidget.ItemSlotWidget_C.OnInputAPressed
// ()

void UItemSlotWidget_C::OnInputAPressed()
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.OnInputAPressed");

	UItemSlotWidget_C_OnInputAPressed_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function ItemSlotWidget.ItemSlotWidget_C.SetupInput_Gamepad
// ()

void UItemSlotWidget_C::SetupInput_Gamepad()
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.SetupInput_Gamepad");

	UItemSlotWidget_C_SetupInput_Gamepad_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function ItemSlotWidget.ItemSlotWidget_C.OnInputX
// ()

void UItemSlotWidget_C::OnInputX()
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.OnInputX");

	UItemSlotWidget_C_OnInputX_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function ItemSlotWidget.ItemSlotWidget_C.CheckSubFocus
// ()
// Parameters:
// bool                           bSubFocus                      (Parm, OutParm, ZeroConstructor, IsPlainOldData)

void UItemSlotWidget_C::CheckSubFocus(bool* bSubFocus)
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.CheckSubFocus");

	UItemSlotWidget_C_CheckSubFocus_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;

	if (bSubFocus != nullptr)
		*bSubFocus = params.bSubFocus;
}


// Function ItemSlotWidget.ItemSlotWidget_C.IsAttachmentFocus
// ()
// Parameters:
// bool                           bIsAttachmentFocus             (Parm, OutParm, ZeroConstructor, IsPlainOldData)

void UItemSlotWidget_C::IsAttachmentFocus(bool* bIsAttachmentFocus)
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.IsAttachmentFocus");

	UItemSlotWidget_C_IsAttachmentFocus_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;

	if (bIsAttachmentFocus != nullptr)
		*bIsAttachmentFocus = params.bIsAttachmentFocus;
}


// Function ItemSlotWidget.ItemSlotWidget_C.IsSubFocus
// ()
// Parameters:
// bool                           bIsSubFocus                    (Parm, OutParm, ZeroConstructor, IsPlainOldData)

void UItemSlotWidget_C::IsSubFocus(bool* bIsSubFocus)
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.IsSubFocus");

	UItemSlotWidget_C_IsSubFocus_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;

	if (bIsSubFocus != nullptr)
		*bIsSubFocus = params.bIsSubFocus;
}


// Function ItemSlotWidget.ItemSlotWidget_C.IsEquipmentFocus
// ()
// Parameters:
// bool                           bIsFocus                       (Parm, OutParm, ZeroConstructor, IsPlainOldData)

void UItemSlotWidget_C::IsEquipmentFocus(bool* bIsFocus)
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.IsEquipmentFocus");

	UItemSlotWidget_C_IsEquipmentFocus_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;

	if (bIsFocus != nullptr)
		*bIsFocus = params.bIsFocus;
}


// Function ItemSlotWidget.ItemSlotWidget_C.GetInventoryWidget_Gamepad
// ()
// Parameters:
// class UInventoryWidget_Gamepad_C* NewParam                       (Parm, OutParm, ZeroConstructor, IsPlainOldData)

void UItemSlotWidget_C::GetInventoryWidget_Gamepad(class UInventoryWidget_Gamepad_C** NewParam)
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.GetInventoryWidget_Gamepad");

	UItemSlotWidget_C_GetInventoryWidget_Gamepad_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;

	if (NewParam != nullptr)
		*NewParam = params.NewParam;
}


// Function ItemSlotWidget.ItemSlotWidget_C.GetFocusSlotID
// ()
// Parameters:
// EEquipSlotID                   EquipSlotID                    (Parm, OutParm, ZeroConstructor, IsPlainOldData)

void UItemSlotWidget_C::GetFocusSlotID(EEquipSlotID* EquipSlotID)
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.GetFocusSlotID");

	UItemSlotWidget_C_GetFocusSlotID_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;

	if (EquipSlotID != nullptr)
		*EquipSlotID = params.EquipSlotID;
}


// Function ItemSlotWidget.ItemSlotWidget_C.SaveSlotInfo
// ()

void UItemSlotWidget_C::SaveSlotInfo()
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.SaveSlotInfo");

	UItemSlotWidget_C_SaveSlotInfo_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function ItemSlotWidget.ItemSlotWidget_C.OnShowToolTip
// ()
// Parameters:
// bool*                          bIsGamepad                     (Parm, ZeroConstructor, IsPlainOldData)

void UItemSlotWidget_C::OnShowToolTip(bool* bIsGamepad)
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.OnShowToolTip");

	UItemSlotWidget_C_OnShowToolTip_Params params;
	params.bIsGamepad = bIsGamepad;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function ItemSlotWidget.ItemSlotWidget_C.ShowStackcountPopup
// ()

void UItemSlotWidget_C::ShowStackcountPopup()
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.ShowStackcountPopup");

	UItemSlotWidget_C_ShowStackcountPopup_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function ItemSlotWidget.ItemSlotWidget_C.OnStackCountpopupBtnDown
// ()
// Parameters:
// EPopupButtonID                 ButtonID                       (Parm, ZeroConstructor, IsPlainOldData)

void UItemSlotWidget_C::OnStackCountpopupBtnDown(EPopupButtonID ButtonID)
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.OnStackCountpopupBtnDown");

	UItemSlotWidget_C_OnStackCountpopupBtnDown_Params params;
	params.ButtonID = ButtonID;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function ItemSlotWidget.ItemSlotWidget_C.On_UseInfo_Text_Prepass_1
// ()
// Parameters:
// class UWidget**                BoundWidget                    (Parm, ZeroConstructor, IsPlainOldData)

void UItemSlotWidget_C::On_UseInfo_Text_Prepass_1(class UWidget** BoundWidget)
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.On_UseInfo_Text_Prepass_1");

	UItemSlotWidget_C_On_UseInfo_Text_Prepass_1_Params params;
	params.BoundWidget = BoundWidget;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function ItemSlotWidget.ItemSlotWidget_C.On_CategoryText_Prepass_1
// ()
// Parameters:
// class UWidget**                BoundWidget                    (Parm, ZeroConstructor, IsPlainOldData)

void UItemSlotWidget_C::On_CategoryText_Prepass_1(class UWidget** BoundWidget)
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.On_CategoryText_Prepass_1");

	UItemSlotWidget_C_On_CategoryText_Prepass_1_Params params;
	params.BoundWidget = BoundWidget;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function ItemSlotWidget.ItemSlotWidget_C.On_CategoryLayer_Prepass_1
// ()
// Parameters:
// class UWidget**                BoundWidget                    (Parm, ZeroConstructor, IsPlainOldData)

void UItemSlotWidget_C::On_CategoryLayer_Prepass_1(class UWidget** BoundWidget)
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.On_CategoryLayer_Prepass_1");

	UItemSlotWidget_C_On_CategoryLayer_Prepass_1_Params params;
	params.BoundWidget = BoundWidget;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function ItemSlotWidget.ItemSlotWidget_C.On_ItemMainLayer_Prepass_1
// ()
// Parameters:
// class UWidget**                BoundWidget                    (Parm, ZeroConstructor, IsPlainOldData)

void UItemSlotWidget_C::On_ItemMainLayer_Prepass_1(class UWidget** BoundWidget)
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.On_ItemMainLayer_Prepass_1");

	UItemSlotWidget_C_On_ItemMainLayer_Prepass_1_Params params;
	params.BoundWidget = BoundWidget;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function ItemSlotWidget.ItemSlotWidget_C.IsCastableItem
// ()
// Parameters:
// bool                           IsCastableItem                 (Parm, OutParm, ZeroConstructor, IsPlainOldData)

void UItemSlotWidget_C::IsCastableItem(bool* IsCastableItem)
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.IsCastableItem");

	UItemSlotWidget_C_IsCastableItem_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;

	if (IsCastableItem != nullptr)
		*IsCastableItem = params.IsCastableItem;
}


// Function ItemSlotWidget.ItemSlotWidget_C.On_DurabilityGauge_Prepass_1
// ()
// Parameters:
// class UWidget**                BoundWidget                    (Parm, ZeroConstructor, IsPlainOldData)

void UItemSlotWidget_C::On_DurabilityGauge_Prepass_1(class UWidget** BoundWidget)
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.On_DurabilityGauge_Prepass_1");

	UItemSlotWidget_C_On_DurabilityGauge_Prepass_1_Params params;
	params.BoundWidget = BoundWidget;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function ItemSlotWidget.ItemSlotWidget_C.On_DurabilityBorder_Prepass_1
// ()
// Parameters:
// class UWidget**                BoundWidget                    (Parm, ZeroConstructor, IsPlainOldData)

void UItemSlotWidget_C::On_DurabilityBorder_Prepass_1(class UWidget** BoundWidget)
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.On_DurabilityBorder_Prepass_1");

	UItemSlotWidget_C_On_DurabilityBorder_Prepass_1_Params params;
	params.BoundWidget = BoundWidget;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function ItemSlotWidget.ItemSlotWidget_C.IsUsableItemSlot
// ()
// Parameters:
// TScriptInterface<class USlotInterface>* Target                         (Parm, ZeroConstructor, IsPlainOldData)
// bool                           ReturnValue                    (Parm, OutParm, ZeroConstructor, ReturnParm, IsPlainOldData)

bool UItemSlotWidget_C::IsUsableItemSlot(TScriptInterface<class USlotInterface>* Target)
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.IsUsableItemSlot");

	UItemSlotWidget_C_IsUsableItemSlot_Params params;
	params.Target = Target;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;

	return params.ReturnValue;
}


// Function ItemSlotWidget.ItemSlotWidget_C.IsAttachableItemSlot
// ()
// Parameters:
// TScriptInterface<class USlotInterface>* Target                         (Parm, ZeroConstructor, IsPlainOldData)
// bool                           return_node                    (Parm, OutParm, ZeroConstructor, IsPlainOldData)

void UItemSlotWidget_C::IsAttachableItemSlot(TScriptInterface<class USlotInterface>* Target, bool* return_node)
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.IsAttachableItemSlot");

	UItemSlotWidget_C_IsAttachableItemSlot_Params params;
	params.Target = Target;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;

	if (return_node != nullptr)
		*return_node = params.return_node;
}


// Function ItemSlotWidget.ItemSlotWidget_C.isPickupableDropItemSlot
// ()
// Parameters:
// TScriptInterface<class USlotInterface>* Target                         (Parm, ZeroConstructor, IsPlainOldData)
// bool                           return_value                   (Parm, OutParm, ZeroConstructor, IsPlainOldData)

void UItemSlotWidget_C::isPickupableDropItemSlot(TScriptInterface<class USlotInterface>* Target, bool* return_value)
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.isPickupableDropItemSlot");

	UItemSlotWidget_C_isPickupableDropItemSlot_Params params;
	params.Target = Target;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;

	if (return_value != nullptr)
		*return_value = params.return_value;
}


// Function ItemSlotWidget.ItemSlotWidget_C.Get_UseInfo_Text_ColorAndOpacity_1
// ()
// Parameters:
// struct FLinearColor            ReturnValue                    (Parm, OutParm, ReturnParm, IsPlainOldData)

struct FLinearColor UItemSlotWidget_C::Get_UseInfo_Text_ColorAndOpacity_1()
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.Get_UseInfo_Text_ColorAndOpacity_1");

	UItemSlotWidget_C_Get_UseInfo_Text_ColorAndOpacity_1_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;

	return params.ReturnValue;
}


// Function ItemSlotWidget.ItemSlotWidget_C.On_CastingBarLayer_Prepass_1
// ()
// Parameters:
// class UWidget**                BoundWidget                    (Parm, ZeroConstructor, IsPlainOldData)

void UItemSlotWidget_C::On_CastingBarLayer_Prepass_1(class UWidget** BoundWidget)
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.On_CastingBarLayer_Prepass_1");

	UItemSlotWidget_C_On_CastingBarLayer_Prepass_1_Params params;
	params.BoundWidget = BoundWidget;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function ItemSlotWidget.ItemSlotWidget_C.GetOperationSpawnValue
// ()
// Parameters:
// class UItem*                   Item                           (Parm, OutParm, ZeroConstructor, IsPlainOldData)
// class UEquipableItem*          EquipableItem                  (Parm, OutParm, ZeroConstructor, IsPlainOldData)
// class UWeaponItem*             WeaponItem                     (Parm, OutParm, ZeroConstructor, IsPlainOldData)
// class UThrowableItem*          ThrowableItem                  (Parm, OutParm, ZeroConstructor, IsPlainOldData)
// class UAttachableItem*         AttachmentItem                 (Parm, OutParm, ZeroConstructor, IsPlainOldData)
// TScriptInterface<class USlotInterface> SlotItem                       (Parm, OutParm, ZeroConstructor, IsPlainOldData)
// TScriptInterface<class USlotContainerInterface> Container                      (Parm, OutParm, ZeroConstructor, IsPlainOldData)

void UItemSlotWidget_C::GetOperationSpawnValue(class UItem** Item, class UEquipableItem** EquipableItem, class UWeaponItem** WeaponItem, class UThrowableItem** ThrowableItem, class UAttachableItem** AttachmentItem, TScriptInterface<class USlotInterface>* SlotItem, TScriptInterface<class USlotContainerInterface>* Container)
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.GetOperationSpawnValue");

	UItemSlotWidget_C_GetOperationSpawnValue_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;

	if (Item != nullptr)
		*Item = params.Item;
	if (EquipableItem != nullptr)
		*EquipableItem = params.EquipableItem;
	if (WeaponItem != nullptr)
		*WeaponItem = params.WeaponItem;
	if (ThrowableItem != nullptr)
		*ThrowableItem = params.ThrowableItem;
	if (AttachmentItem != nullptr)
		*AttachmentItem = params.AttachmentItem;
	if (SlotItem != nullptr)
		*SlotItem = params.SlotItem;
	if (Container != nullptr)
		*Container = params.Container;
}


// Function ItemSlotWidget.ItemSlotWidget_C.UpdateItemIfoVisibility
// ()
// Parameters:
// class UWidget**                BoundWidget                    (Parm, ZeroConstructor, IsPlainOldData)

void UItemSlotWidget_C::UpdateItemIfoVisibility(class UWidget** BoundWidget)
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.UpdateItemIfoVisibility");

	UItemSlotWidget_C_UpdateItemIfoVisibility_Params params;
	params.BoundWidget = BoundWidget;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function ItemSlotWidget.ItemSlotWidget_C.UpdateInnerItemSlotListVisibility
// ()
// Parameters:
// class UWidget**                BoundWidget                    (Parm, ZeroConstructor, IsPlainOldData)

void UItemSlotWidget_C::UpdateInnerItemSlotListVisibility(class UWidget** BoundWidget)
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.UpdateInnerItemSlotListVisibility");

	UItemSlotWidget_C_UpdateInnerItemSlotListVisibility_Params params;
	params.BoundWidget = BoundWidget;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function ItemSlotWidget.ItemSlotWidget_C.UpdateOpenButtonVisibility
// ()
// Parameters:
// class UWidget**                BoundWidget                    (Parm, ZeroConstructor, IsPlainOldData)

void UItemSlotWidget_C::UpdateOpenButtonVisibility(class UWidget** BoundWidget)
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.UpdateOpenButtonVisibility");

	UItemSlotWidget_C_UpdateOpenButtonVisibility_Params params;
	params.BoundWidget = BoundWidget;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function ItemSlotWidget.ItemSlotWidget_C.UpdateEquipableIconVisibility
// ()
// Parameters:
// class UWidget**                BoundWidget                    (Parm, ZeroConstructor, IsPlainOldData)

void UItemSlotWidget_C::UpdateEquipableIconVisibility(class UWidget** BoundWidget)
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.UpdateEquipableIconVisibility");

	UItemSlotWidget_C_UpdateEquipableIconVisibility_Params params;
	params.BoundWidget = BoundWidget;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function ItemSlotWidget.ItemSlotWidget_C.UpdateUseButtonVisibility
// ()
// Parameters:
// class UWidget**                BoundWidget                    (Parm, ZeroConstructor, IsPlainOldData)

void UItemSlotWidget_C::UpdateUseButtonVisibility(class UWidget** BoundWidget)
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.UpdateUseButtonVisibility");

	UItemSlotWidget_C_UpdateUseButtonVisibility_Params params;
	params.BoundWidget = BoundWidget;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function ItemSlotWidget.ItemSlotWidget_C.UpdateStackCountVisibility
// ()
// Parameters:
// class UWidget**                BoundWidget                    (Parm, ZeroConstructor, IsPlainOldData)

void UItemSlotWidget_C::UpdateStackCountVisibility(class UWidget** BoundWidget)
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.UpdateStackCountVisibility");

	UItemSlotWidget_C_UpdateStackCountVisibility_Params params;
	params.BoundWidget = BoundWidget;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function ItemSlotWidget.ItemSlotWidget_C.DialogButtonDown
// ()
// Parameters:
// EPopupButtonID                 ButtonID                       (Parm, ZeroConstructor, IsPlainOldData)

void UItemSlotWidget_C::DialogButtonDown(EPopupButtonID ButtonID)
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.DialogButtonDown");

	UItemSlotWidget_C_DialogButtonDown_Params params;
	params.ButtonID = ButtonID;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function ItemSlotWidget.ItemSlotWidget_C.On_Brackground_Prepass_1
// ()
// Parameters:
// class UWidget**                BoundWidget                    (Parm, ZeroConstructor, IsPlainOldData)

void UItemSlotWidget_C::On_Brackground_Prepass_1(class UWidget** BoundWidget)
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.On_Brackground_Prepass_1");

	UItemSlotWidget_C_On_Brackground_Prepass_1_Params params;
	params.BoundWidget = BoundWidget;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function ItemSlotWidget.ItemSlotWidget_C.UpdateIconSizeRatio
// ()
// Parameters:
// class UWidget**                BoundWidget                    (Parm, ZeroConstructor, IsPlainOldData)

void UItemSlotWidget_C::UpdateIconSizeRatio(class UWidget** BoundWidget)
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.UpdateIconSizeRatio");

	UItemSlotWidget_C_UpdateIconSizeRatio_Params params;
	params.BoundWidget = BoundWidget;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function ItemSlotWidget.ItemSlotWidget_C.UpdateUseText
// ()
// Parameters:
// class UWidget**                BoundWidget                    (Parm, ZeroConstructor, IsPlainOldData)

void UItemSlotWidget_C::UpdateUseText(class UWidget** BoundWidget)
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.UpdateUseText");

	UItemSlotWidget_C_UpdateUseText_Params params;
	params.BoundWidget = BoundWidget;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function ItemSlotWidget.ItemSlotWidget_C.UpdateButtonIconIndex
// ()
// Parameters:
// class UWidget**                BoundWidget                    (Parm, ZeroConstructor, IsPlainOldData)

void UItemSlotWidget_C::UpdateButtonIconIndex(class UWidget** BoundWidget)
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.UpdateButtonIconIndex");

	UItemSlotWidget_C_UpdateButtonIconIndex_Params params;
	params.BoundWidget = BoundWidget;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function ItemSlotWidget.ItemSlotWidget_C.UpdateSlot
// ()
// Parameters:
// TScriptInterface<class USlotInterface>* Item_SlotInterface             (Parm, ZeroConstructor, IsPlainOldData)
// TScriptInterface<class USlotContainerInterface>* SlotContainer                  (Parm, ZeroConstructor, IsPlainOldData)
// bool*                          bIsPackaged                    (Parm, ZeroConstructor, IsPlainOldData)
// bool*                          bSort                          (Parm, ZeroConstructor, IsPlainOldData)

void UItemSlotWidget_C::UpdateSlot(TScriptInterface<class USlotInterface>* Item_SlotInterface, TScriptInterface<class USlotContainerInterface>* SlotContainer, bool* bIsPackaged, bool* bSort)
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.UpdateSlot");

	UItemSlotWidget_C_UpdateSlot_Params params;
	params.Item_SlotInterface = Item_SlotInterface;
	params.SlotContainer = SlotContainer;
	params.bIsPackaged = bIsPackaged;
	params.bSort = bSort;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function ItemSlotWidget.ItemSlotWidget_C.UpdateItemVisibility
// ()
// Parameters:
// class UWidget**                BoundWidget                    (Parm, ZeroConstructor, IsPlainOldData)

void UItemSlotWidget_C::UpdateItemVisibility(class UWidget** BoundWidget)
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.UpdateItemVisibility");

	UItemSlotWidget_C_UpdateItemVisibility_Params params;
	params.BoundWidget = BoundWidget;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function ItemSlotWidget.ItemSlotWidget_C.UpdateStackCountText
// ()
// Parameters:
// class UWidget**                BoundWidget                    (Parm, ZeroConstructor, IsPlainOldData)

void UItemSlotWidget_C::UpdateStackCountText(class UWidget** BoundWidget)
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.UpdateStackCountText");

	UItemSlotWidget_C_UpdateStackCountText_Params params;
	params.BoundWidget = BoundWidget;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function ItemSlotWidget.ItemSlotWidget_C.UpdateItemIconImage
// ()
// Parameters:
// class UWidget**                BoundWidget                    (Parm, ZeroConstructor, IsPlainOldData)

void UItemSlotWidget_C::UpdateItemIconImage(class UWidget** BoundWidget)
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.UpdateItemIconImage");

	UItemSlotWidget_C_UpdateItemIconImage_Params params;
	params.BoundWidget = BoundWidget;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function ItemSlotWidget.ItemSlotWidget_C.UpdateSlotName
// ()
// Parameters:
// class UWidget**                BoundWidget                    (Parm, ZeroConstructor, IsPlainOldData)

void UItemSlotWidget_C::UpdateSlotName(class UWidget** BoundWidget)
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.UpdateSlotName");

	UItemSlotWidget_C_UpdateSlotName_Params params;
	params.BoundWidget = BoundWidget;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function ItemSlotWidget.ItemSlotWidget_C.GetItemCount
// ()
// Parameters:
// int                            Count                          (Parm, OutParm, ZeroConstructor, IsPlainOldData)

void UItemSlotWidget_C::GetItemCount(int* Count)
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.GetItemCount");

	UItemSlotWidget_C_GetItemCount_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;

	if (Count != nullptr)
		*Count = params.Count;
}


// Function ItemSlotWidget.ItemSlotWidget_C.CreateTslDragDropOperation
// ()
// Parameters:
// class UTslItemDragDropOperation_C* OutOperation                   (Parm, OutParm, ZeroConstructor, IsPlainOldData)

void UItemSlotWidget_C::CreateTslDragDropOperation(class UTslItemDragDropOperation_C** OutOperation)
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.CreateTslDragDropOperation");

	UItemSlotWidget_C_CreateTslDragDropOperation_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;

	if (OutOperation != nullptr)
		*OutOperation = params.OutOperation;
}


// Function ItemSlotWidget.ItemSlotWidget_C.GetOpenButtonVisibility
// ()
// Parameters:
// ESlateVisibility               ReturnValue                    (Parm, OutParm, ZeroConstructor, ReturnParm, IsPlainOldData)

ESlateVisibility UItemSlotWidget_C::GetOpenButtonVisibility()
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.GetOpenButtonVisibility");

	UItemSlotWidget_C_GetOpenButtonVisibility_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;

	return params.ReturnValue;
}


// Function ItemSlotWidget.ItemSlotWidget_C.DispatchRefreshInnerList
// ()

void UItemSlotWidget_C::DispatchRefreshInnerList()
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.DispatchRefreshInnerList");

	UItemSlotWidget_C_DispatchRefreshInnerList_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function ItemSlotWidget.ItemSlotWidget_C.IsDraggable
// ()
// Parameters:
// bool                           Result                         (Parm, OutParm, ZeroConstructor, IsPlainOldData)

void UItemSlotWidget_C::IsDraggable(bool* Result)
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.IsDraggable");

	UItemSlotWidget_C_IsDraggable_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;

	if (Result != nullptr)
		*Result = params.Result;
}


// Function ItemSlotWidget.ItemSlotWidget_C.OnDragDetected
// (BlueprintCosmetic, Event)
// Parameters:
// struct FGeometry*              MyGeometry                     (Parm, IsPlainOldData)
// struct FPointerEvent*          PointerEvent                   (ConstParm, Parm, OutParm, ReferenceParm)
// class UDragDropOperation*      Operation                      (Parm, OutParm, ZeroConstructor, IsPlainOldData)

void UItemSlotWidget_C::OnDragDetected(struct FGeometry* MyGeometry, struct FPointerEvent* PointerEvent, class UDragDropOperation** Operation)
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.OnDragDetected");

	UItemSlotWidget_C_OnDragDetected_Params params;
	params.MyGeometry = MyGeometry;
	params.PointerEvent = PointerEvent;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;

	if (Operation != nullptr)
		*Operation = params.Operation;
}


// Function ItemSlotWidget.ItemSlotWidget_C.OnMouseButtonDown
// (BlueprintCosmetic, Event)
// Parameters:
// struct FGeometry*              MyGeometry                     (Parm, IsPlainOldData)
// struct FPointerEvent*          MouseEvent                     (ConstParm, Parm, OutParm, ReferenceParm)
// struct FEventReply             ReturnValue                    (Parm, OutParm, ReturnParm)

struct FEventReply UItemSlotWidget_C::OnMouseButtonDown(struct FGeometry* MyGeometry, struct FPointerEvent* MouseEvent)
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.OnMouseButtonDown");

	UItemSlotWidget_C_OnMouseButtonDown_Params params;
	params.MyGeometry = MyGeometry;
	params.MouseEvent = MouseEvent;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;

	return params.ReturnValue;
}


// Function ItemSlotWidget.ItemSlotWidget_C.GetContentCount
// ()
// Parameters:
// int                            Count                          (Parm, OutParm, ZeroConstructor, IsPlainOldData)

void UItemSlotWidget_C::GetContentCount(int* Count)
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.GetContentCount");

	UItemSlotWidget_C_GetContentCount_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;

	if (Count != nullptr)
		*Count = params.Count;
}


// Function ItemSlotWidget.ItemSlotWidget_C.Refresh
// ()

void UItemSlotWidget_C::Refresh()
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.Refresh");

	UItemSlotWidget_C_Refresh_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function ItemSlotWidget.ItemSlotWidget_C.GetInnerContainerVisibility
// ()
// Parameters:
// ESlateVisibility               ReturnValue                    (Parm, OutParm, ZeroConstructor, ReturnParm, IsPlainOldData)

ESlateVisibility UItemSlotWidget_C::GetInnerContainerVisibility()
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.GetInnerContainerVisibility");

	UItemSlotWidget_C_GetInnerContainerVisibility_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;

	return params.ReturnValue;
}


// Function ItemSlotWidget.ItemSlotWidget_C.GetPackagedLabelVisibility
// ()
// Parameters:
// ESlateVisibility               ReturnValue                    (Parm, OutParm, ZeroConstructor, ReturnParm, IsPlainOldData)

ESlateVisibility UItemSlotWidget_C::GetPackagedLabelVisibility()
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.GetPackagedLabelVisibility");

	UItemSlotWidget_C_GetPackagedLabelVisibility_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;

	return params.ReturnValue;
}


// Function ItemSlotWidget.ItemSlotWidget_C.GetIconVisibility
// ()
// Parameters:
// ESlateVisibility               ReturnValue                    (Parm, OutParm, ZeroConstructor, ReturnParm, IsPlainOldData)

ESlateVisibility UItemSlotWidget_C::GetIconVisibility()
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.GetIconVisibility");

	UItemSlotWidget_C_GetIconVisibility_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;

	return params.ReturnValue;
}


// Function ItemSlotWidget.ItemSlotWidget_C.GetAmmountTextVisibility
// ()
// Parameters:
// ESlateVisibility               ReturnValue                    (Parm, OutParm, ZeroConstructor, ReturnParm, IsPlainOldData)

ESlateVisibility UItemSlotWidget_C::GetAmmountTextVisibility()
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.GetAmmountTextVisibility");

	UItemSlotWidget_C_GetAmmountTextVisibility_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;

	return params.ReturnValue;
}


// Function ItemSlotWidget.ItemSlotWidget_C.GetAmmountText
// ()
// Parameters:
// struct FText                   ReturnValue                    (Parm, OutParm, ReturnParm)

struct FText UItemSlotWidget_C::GetAmmountText()
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.GetAmmountText");

	UItemSlotWidget_C_GetAmmountText_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;

	return params.ReturnValue;
}


// Function ItemSlotWidget.ItemSlotWidget_C.GetUseButtonVisibility
// ()
// Parameters:
// ESlateVisibility               ReturnValue                    (Parm, OutParm, ZeroConstructor, ReturnParm, IsPlainOldData)

ESlateVisibility UItemSlotWidget_C::GetUseButtonVisibility()
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.GetUseButtonVisibility");

	UItemSlotWidget_C_GetUseButtonVisibility_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;

	return params.ReturnValue;
}


// Function ItemSlotWidget.ItemSlotWidget_C.GetIconTexture
// ()
// Parameters:
// class UTexture*                Tex                            (Parm, OutParm, ZeroConstructor, IsPlainOldData)

void UItemSlotWidget_C::GetIconTexture(class UTexture** Tex)
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.GetIconTexture");

	UItemSlotWidget_C_GetIconTexture_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;

	if (Tex != nullptr)
		*Tex = params.Tex;
}


// Function ItemSlotWidget.ItemSlotWidget_C.GetSlotName
// ()
// Parameters:
// struct FText                   ReturnValue                    (Parm, OutParm, ReturnParm)

struct FText UItemSlotWidget_C::GetSlotName()
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.GetSlotName");

	UItemSlotWidget_C_GetSlotName_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;

	return params.ReturnValue;
}


// Function ItemSlotWidget.ItemSlotWidget_C.OnMouseEnter
// (BlueprintCosmetic, Event)
// Parameters:
// struct FGeometry               MyGeometry                     (Parm, IsPlainOldData)
// struct FPointerEvent           MouseEvent                     (ConstParm, Parm, OutParm, ReferenceParm)

void UItemSlotWidget_C::OnMouseEnter(const struct FGeometry& MyGeometry, const struct FPointerEvent& MouseEvent)
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.OnMouseEnter");

	UItemSlotWidget_C_OnMouseEnter_Params params;
	params.MyGeometry = MyGeometry;
	params.MouseEvent = MouseEvent;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function ItemSlotWidget.ItemSlotWidget_C.Destruct
// (BlueprintCosmetic, Event)

void UItemSlotWidget_C::Destruct()
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.Destruct");

	UItemSlotWidget_C_Destruct_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function ItemSlotWidget.ItemSlotWidget_C.OnMouseLeave
// (BlueprintCosmetic, Event)
// Parameters:
// struct FPointerEvent           MouseEvent                     (ConstParm, Parm, OutParm, ReferenceParm)

void UItemSlotWidget_C::OnMouseLeave(const struct FPointerEvent& MouseEvent)
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.OnMouseLeave");

	UItemSlotWidget_C_OnMouseLeave_Params params;
	params.MouseEvent = MouseEvent;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function ItemSlotWidget.ItemSlotWidget_C.BndEvt__InnerItemSlotListWidget_K2Node_ComponentBoundEvent_0_OnChildSlotFocus__DelegateSignature
// ()
// Parameters:
// TScriptInterface<class USlotInterface>* Slot                           (Parm, ZeroConstructor, IsPlainOldData)

void UItemSlotWidget_C::BndEvt__InnerItemSlotListWidget_K2Node_ComponentBoundEvent_0_OnChildSlotFocus__DelegateSignature(TScriptInterface<class USlotInterface>* Slot)
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.BndEvt__InnerItemSlotListWidget_K2Node_ComponentBoundEvent_0_OnChildSlotFocus__DelegateSignature");

	UItemSlotWidget_C_BndEvt__InnerItemSlotListWidget_K2Node_ComponentBoundEvent_0_OnChildSlotFocus__DelegateSignature_Params params;
	params.Slot = Slot;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function ItemSlotWidget.ItemSlotWidget_C.Construct
// (BlueprintCosmetic, Event)

void UItemSlotWidget_C::Construct()
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.Construct");

	UItemSlotWidget_C_Construct_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function ItemSlotWidget.ItemSlotWidget_C.Tick
// (BlueprintCosmetic, Event)
// Parameters:
// struct FGeometry               MyGeometry                     (Parm, IsPlainOldData)
// float                          InDeltaTime                    (Parm, ZeroConstructor, IsPlainOldData)

void UItemSlotWidget_C::Tick(const struct FGeometry& MyGeometry, float InDeltaTime)
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.Tick");

	UItemSlotWidget_C_Tick_Params params;
	params.MyGeometry = MyGeometry;
	params.InDeltaTime = InDeltaTime;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function ItemSlotWidget.ItemSlotWidget_C.OnDragEnter
// (BlueprintCosmetic, Event)
// Parameters:
// struct FGeometry*              MyGeometry                     (Parm, IsPlainOldData)
// struct FPointerEvent*          PointerEvent                   (Parm)
// class UDragDropOperation**     Operation                      (Parm, ZeroConstructor, IsPlainOldData)

void UItemSlotWidget_C::OnDragEnter(struct FGeometry* MyGeometry, struct FPointerEvent* PointerEvent, class UDragDropOperation** Operation)
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.OnDragEnter");

	UItemSlotWidget_C_OnDragEnter_Params params;
	params.MyGeometry = MyGeometry;
	params.PointerEvent = PointerEvent;
	params.Operation = Operation;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function ItemSlotWidget.ItemSlotWidget_C.ExecuteUbergraph_ItemSlotWidget
// ()
// Parameters:
// int*                           EntryPoint                     (Parm, ZeroConstructor, IsPlainOldData)

void UItemSlotWidget_C::ExecuteUbergraph_ItemSlotWidget(int* EntryPoint)
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.ExecuteUbergraph_ItemSlotWidget");

	UItemSlotWidget_C_ExecuteUbergraph_ItemSlotWidget_Params params;
	params.EntryPoint = EntryPoint;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function ItemSlotWidget.ItemSlotWidget_C.OnLeaveSlot__DelegateSignature
// ()
// Parameters:
// TScriptInterface<class USlotInterface>* Slot                           (Parm, ZeroConstructor, IsPlainOldData)
// TScriptInterface<class USlotContainerInterface>* SlotContainer                  (Parm, ZeroConstructor, IsPlainOldData)

void UItemSlotWidget_C::OnLeaveSlot__DelegateSignature(TScriptInterface<class USlotInterface>* Slot, TScriptInterface<class USlotContainerInterface>* SlotContainer)
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.OnLeaveSlot__DelegateSignature");

	UItemSlotWidget_C_OnLeaveSlot__DelegateSignature_Params params;
	params.Slot = Slot;
	params.SlotContainer = SlotContainer;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function ItemSlotWidget.ItemSlotWidget_C.OnEnterSlot__DelegateSignature
// ()
// Parameters:
// TScriptInterface<class USlotInterface>* Slot                           (Parm, ZeroConstructor, IsPlainOldData)
// TScriptInterface<class USlotContainerInterface>* SlotContainer                  (Parm, ZeroConstructor, IsPlainOldData)

void UItemSlotWidget_C::OnEnterSlot__DelegateSignature(TScriptInterface<class USlotInterface>* Slot, TScriptInterface<class USlotContainerInterface>* SlotContainer)
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.OnEnterSlot__DelegateSignature");

	UItemSlotWidget_C_OnEnterSlot__DelegateSignature_Params params;
	params.Slot = Slot;
	params.SlotContainer = SlotContainer;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function ItemSlotWidget.ItemSlotWidget_C.OnDoSlotAction__DelegateSignature
// ()

void UItemSlotWidget_C::OnDoSlotAction__DelegateSignature()
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.OnDoSlotAction__DelegateSignature");

	UItemSlotWidget_C_OnDoSlotAction__DelegateSignature_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function ItemSlotWidget.ItemSlotWidget_C.RefreshFocus__DelegateSignature
// ()

void UItemSlotWidget_C::RefreshFocus__DelegateSignature()
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.RefreshFocus__DelegateSignature");

	UItemSlotWidget_C_RefreshFocus__DelegateSignature_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function ItemSlotWidget.ItemSlotWidget_C.OnRefreshInnerList__DelegateSignature
// ()

void UItemSlotWidget_C::OnRefreshInnerList__DelegateSignature()
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.OnRefreshInnerList__DelegateSignature");

	UItemSlotWidget_C_OnRefreshInnerList__DelegateSignature_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function ItemSlotWidget.ItemSlotWidget_C.OnSlotFocus__DelegateSignature
// ()
// Parameters:
// TScriptInterface<class USlotInterface>* Slot                           (Parm, ZeroConstructor, IsPlainOldData)
// TScriptInterface<class USlotContainerInterface>* SlotContainer                  (Parm, ZeroConstructor, IsPlainOldData)

void UItemSlotWidget_C::OnSlotFocus__DelegateSignature(TScriptInterface<class USlotInterface>* Slot, TScriptInterface<class USlotContainerInterface>* SlotContainer)
{
	static auto fn = UObject::FindObject<UFunction>("Function ItemSlotWidget.ItemSlotWidget_C.OnSlotFocus__DelegateSignature");

	UItemSlotWidget_C_OnSlotFocus__DelegateSignature_Params params;
	params.Slot = Slot;
	params.SlotContainer = SlotContainer;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


}

#ifdef _MSC_VER
	#pragma pack(pop)
#endif
